...
class FeatureDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_freq_fert:
            ...
            return self.features[idx].cuda(), self.targets[idx].cuda(), self.knn_probs[idx].cuda(), self.network_probs[idx].cuda(), self.conf[idx].cuda().unsqueeze(-1), self.ent[idx].cuda().unsqueeze(-1), self.min_dist[idx].cuda(), self.min_top32_dist[idx].cuda()
        else:
